...
-   First, looking at the model coefficients alone: 1. `pctpov` = 0.012, percent poverty alone increases probability of `violation`, 2. `pcthisp` = -0.0056, percent Hispanic alone slightly *decreases* probability of `violation`, 3. `pctpov:pcthisp` = 0.0003, the interaction of poverty and race results in a steeper rise in `violation` probability. Communities that are more poor and more Hispanic face greater risks for SDWA violations. Quantitatively, this is seen in the positive interaction coefficient ($0.0003 \pm 0.00006$ at a 95% CI). In the figure, this corresponds to the low probabilities at low poverty (0-25%) and the higher probabilities at high poverty (50-75%), the predictions become much steeper as Hispanic percentage increases. One way these outputs could be misinterpreted is one could say "poor & Hispanic communities *cause* the poor drinking water in their communities". Instead of the saying predictors are causing the poor water quality, one could say that the environmental risk is socially patterned as there is a significant correlation between our predictors and response.
